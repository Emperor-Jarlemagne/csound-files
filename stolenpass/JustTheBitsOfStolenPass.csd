<CsoundSynthesizer>
<CsOptions>
-o dac -m4
</CsOptions>
<CsInstruments>
sr = 44100
ksmps = 64
nchnls = 2
0dbfs = 1
seed 0

gkTempo init (120 / 60) * 4
giSin ftgen 1, 0, 1024, 10, 1
gicos ftgen 0, 0, 65536, 9, 1, 90
giSample ftgen 2, 0, 0, 1, "kick1.aif", 0, 4, 1
schedule "clock", 0, -1
schedule "reverb", 0, -1
schedule "floating_gate", 0, -1

instr clock
  ktrig metro gkTempo
  kcount init 0
  if ktrig == 1 then
    schedulek "schedule", .0, .1, kcount
    kcount += 1
  endif
endin

instr schedule
  icount = p4
  if icount % 1 == 0 then
     schedule "_init_sound", .0, .3
     schedule "_bass", .0, .3
  endif
  if icount % 1 == 0 then
     schedule "_hihat", .0, random(.03, .13)
  endif
  if (icount % 8 == 0) then
     schedule "_init_sound", .0, (1/i(gkTempo)) * 6.
     schedule "_kick", .0, .5
;     schedule "_kick", .25, .5
     schedule "_kick", .625, .5
     schedule "_kick", .875, .5
  endif
endin

instr _init_sound
    ifreq = 110
    aenv linseg .0, p3*.1, .2, p3*.9, .0
    amod oscil ifreq, ifreq
    asig oscil aenv, ifreq + amod
    outs asig, asig
  ifreq = 110 * int(random(1,3))
  iratio = .5 * int(random(1,14))
  indx = 2.
  iattk = random(.0, 1.0) > .5 ? .001 : .9
  aenv linseg .0, p3*iattk, .2, p3*(1-iattk), .0
  amod oscil ifreq * iratio * indx, ifreq * iratio
  asig oscil aenv, ifreq + amod
  al, ar pan2 asig, .5 + random(-.4, .4)
  outs al, ar
  chnmix al*.3, "reverbL"
  chnmix ar*.3, "reverbR"
endin

instr _kick
  aindx line .0, p3, ftsr(giSample)*p3
  asig table aindx, giSample
  asig = asig*.7
  outs asig, asig
endin

instr _hihat
  asig noise line:a(.24, p3, .0), .0
  outs asig, asig
endin

instr _bass
  asig vco line:a(.2, p3, .0), 110, 1, 0, giSin
  asig = lowpass2(asig, 400 + 400*int(random(1,10)), 4)
  outs asig, asig
  chnmix asig*.2, "reverbL"
  chnmix asig*.2, "reverbR"
endin

instr _snare
	ifreq = 342
	 apitch1 oscili 1, ifreq*octave(gicos)
	 apitch2 oscili 0.25, ifreq*0.5*octave(gicos)
	 apitch = (apitch1 + apitch2)*0.75           
    aenv2    expon    1,p3,0.0005
    anoise    noise    0.75,0       
    anoise    buthp    anoise,1000                    
    kcf    expseg    5000,0.1,3000,p3-0.2,3000            
    anoise    butlp    anoise,kcf
    aOut = (apitch*aenv2)+anoise                    
        outs    aOut,aOut  
endin

instr reverb
  al, ar reverbsc chnget:a("reverbL"), chnget:a("reverbR"), .9, 12000
  outs al, ar
  chnclear "reverbL", "reverbR"
endin

instr floating_gate
	kGate randomi 0, 1, 3, 3
	kdb	randomi -12, 0, 5, 3
	aSig = (giSin * 2)/.5 
	kVol init 0
	kVol = (kGate > 0.5 ? ampdb(kdb) : 0)
	kVol port kVol, .02
	aOut = aSig * kVol
		outs aOut, aOut
endin

</CsInstruments>
<CsScore>
i 1 0 20
</CsScore>
</CsoundSynthesizer>
<bsbPanel>
 <label>Widgets</label>
 <objectName/>
 <x>100</x>
 <y>100</y>
 <width>320</width>
 <height>240</height>
 <visible>true</visible>
 <uuid/>
 <bgcolor mode="background">
  <r>240</r>
  <g>240</g>
  <b>240</b>
 </bgcolor>
</bsbPanel>
<bsbPresets>
</bsbPresets>
